...
async function runTests() {
  ...
  // T3.4 & T3.5 - Valid Form Submit & Reset
  if (quoteForm) {
    ...
    assert(
      ...
      'T3.4',
      'Valid form submission triggers success modal/toast (#success-toast.active)',
      'tier3'
    );
  ...
  // T3.7 & T3.8 - Gallery Item Click & Lightbox Modal
  ...
  // T3.9 - Lightbox Backdrop Click Close
  if (galleryItems.length > 0 && lightboxModal) {
  ...
  // T3.10 - Escape Key Closes Lightbox
  if (galleryItems.length > 0 && lightboxModal) {
    galleryItems[0].click();
